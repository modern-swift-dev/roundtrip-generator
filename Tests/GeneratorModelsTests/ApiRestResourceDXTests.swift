...
struct ApiRestResourceDXTests {
    ...
    private var user: ApiTypeSchema {
        .object("User") { .string("name") }
    }

    @Test func `defaults preserve conventional contract`() {
    ...
    @Test func `typed identifiers use matching model and parameter types`() {
        ...
        for identifier in cases {
            ...
            switch identifier.kind {
                case .int64:
                    ...
                    guard case .int64 = parameter.dataType else {
                        Issue.record("Expected Int64 path parameter"); continue
                    }
                case .string:
                    #expect(resource.identifierProperty.dataType == .string())
                    guard case .string = parameter.dataType else {
                        Issue.record("Expected string path parameter"); continue
                    }
                case .uuid:
                    #expect(resource.identifierProperty.dataType == .uuid)
                    #expect(parameter.rawName == "user_key")
                    guard case .string = parameter.dataType else {
                        Issue.record("Expected UUID encoded as string path parameter"); continue
                    }
            }
        }
    }

    @Test func `read only security and array pagination are explicit`() {
    ...
    @Test func `nested paths and custom operations honor identifier overrides`() {
