...
struct ApiCollectionBuilderTests {
    @Test func `property builder composes reusable arrays conditionals and loops`() throws {
        ...
        let model = ApiTypeSchema.object("User") {
            ...
            if includeHidden {
                ApiModelProperty.string("excluded")
            }
            for name in ["email", "phone"] {
                ApiModelProperty.string(name).optional
            }
        }
    ...
    @Test func `nested package declarations preserve references and operations`() {
    ...
    @Test func `empty blocks and resource groups compile`() {
