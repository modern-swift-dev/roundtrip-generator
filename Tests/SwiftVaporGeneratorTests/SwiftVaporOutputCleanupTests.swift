...
struct SwiftVaporOutputCleanupTests {
    @Test func `aliased output preserves current files and removes stale files`() throws {
        ...
        let generator = SwiftVaporApiPackageGenerator(package: package)
        let files = try generator.generatedFiles()
        let source = try #require(files.first { $0.relativePath.contains("/Generated/") && $0.relativePath.hasSuffix(".swift") })
        let stale = alias.appendingPathComponent(source.relativePath).deletingLastPathComponent().appendingPathComponent("Obsolete.swift")
