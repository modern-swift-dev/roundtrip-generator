...
struct KotlinAndroidProjectTests {
    @Test func `keyword base packages fail before writing kotlin`() {
    ...
    @Test func `custom project coordinates and package paths are escaped`() throws {
    ...
    @Test func `disabling all optional sources retains models and services`() throws {
        let options = KotlinAndroidGeneratorOptions(
    ...
    @Test func `manifest permission and managed xml respect never overwrite policy`() throws {
    ...
    @Test func `directory symlink contained in output root can be written`() throws {
        let root = temporaryRoot()
        defer { try? FileManager.default.removeItem(at: root) }
    ...
    @Test func `generated leaf symlink cannot replace an outside managed source`() throws {
        let root = temporaryRoot()
        defer { try? FileManager.default.removeItem(at: root) }
    ...
    @Test func `never overwrite refuses even managed gradle files`() throws {
        let root = temporaryRoot()
        defer { try? FileManager.default.removeItem(at: root) }
    ...
    @Test func `stale leaf symlink cannot delete an outside managed source`() throws {
        let root = temporaryRoot()
        defer { try? FileManager.default.removeItem(at: root) }
        let output = root.appendingPathComponent("output", isDirectory: true)
