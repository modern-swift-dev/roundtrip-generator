...
struct KotlinSpringBootProjectLayoutTests {
    @Test func `standalone preset preserves default options`() {
        #expect(KotlinSpringBootGeneratorOptions.standaloneProject() == KotlinSpringBootGeneratorOptions())
    }
    ...
    @Test func `existing project preserves handwritten scaffolding and writes only sources`() throws {
        ...
        let generator = KotlinSpringBootApiPackageGenerator(package: package, options: .existingProject())
    ...
    @Test func `adding mapping preserves project layout`() {
        let options = KotlinSpringBootGeneratorOptions.existingProject().mapping(.init(apiTypeName: "External", kotlinType: "External"))
