...
struct SwiftVaporModelEmitter {
    ...
    private func objectDeclaration(
        ...
        return SwiftGeneratedSyntax.parse("model \(name)") {
            if isValueType {
                try DeclSyntax(StructDeclSyntax("public struct \(raw: name): \(raw: conformances.joined(separator: ", "))") {
                    members
                })
            } else {
                try DeclSyntax(ClassDeclSyntax("public class \(raw: name): \(raw: conformances.joined(separator: ", "))") {
                    members
                })
            }
        }
    ...
    private func dynamicObjectDecodeCustomTypeDeclaration() -> FunctionDeclSyntax {
        SwiftGeneratedSyntax.parse("dynamicObjectDecodeCustomTypeDeclaration") {
            try FunctionDeclSyntax(
                """
...
private extension ApiModelProperty {
    ...
    func swiftVaporDefaultValue(registry _: SwiftVaporTypeRegistry) -> String? {
