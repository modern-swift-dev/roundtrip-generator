...
struct KotlinAndroidOperationEmitter {
    let operation: ApiOperation
    let options: KotlinAndroidGeneratorOptions

    init(operation: ApiOperation, options: KotlinAndroidGeneratorOptions = .init()) {
    ...
    func serviceMethodDeclaration(includeProgressDefault: Bool = true) -> String {
        methodSignature(name: operation.kotlinMethodName, parameters: serviceParameters(includeDefaults: includeProgressDefault))
    }

    func nextPageMethodDeclaration(includeDefaults: Bool = true) -> String {
        let responseType = operation.response.dataType?.kotlinTypeDeclaration(options: options) ?? "Unit"
        return methodSignature(
            name: "getNextPageFor\(operation.kotlinOperationTypeName)",
            parameters: ["currentPage: \(responseType)"] + serviceParameters(includeDefaults: includeDefaults, includeProgress: false)
        )
    }

    private func methodSignature(name: String, parameters: [String]) -> String {
        Block {
            "suspend fun \(name)("
            Indentation { NodeList(parameters.map { "\($0)," as any Node }) }
            "): \(serviceReturnType)"
        }.toString()
    }

    private func serviceParameters(includeDefaults: Bool, includeProgress: Bool = true) -> [String] {
        let runtime = operation.path.isRuntime ? ["requestUrl: String"] : []
        let properties = operation.expandedParameters.map(requestProperty) + requestBodyProperty()
        var parameters = runtime + properties.map { property in
            let declaration = String(property.dropFirst(4))
            return includeDefaults ? declaration : String(declaration.components(separatedBy: " =")[0])
        }
        if includeProgress && operation.request.requiresProgress {
            parameters.append("progress: ApiProgress?" + (includeDefaults ? " = null" : ""))
        }
        return parameters
    }

    func requestConstruction() -> String {
        let runtime = operation.path.isRuntime ? ["requestUrl"] : []
        let parameters = operation.expandedParameters.map { $0.propertyName.kotlinPropertyName }
        let body = operation.request.hasBody ? ["body"] : []
        let arguments = (runtime + parameters + body).map { "\($0) = \($0)" }.joined(separator: ", ")
        return "val request = \(operation.kotlinOperationTypeName).Request(\(arguments))"
    }
    ...
    func serviceMethodImplementation() -> any Node {
        ...
        if let responseType = operation.response.dataType {
            ...
            callArguments.append(
                "responseType = ApiResponseType<\(typeDeclaration)>(typeName = \(typeDeclaration.kotlinStringLiteral), mimeType = \(operation.response.mimeType.kotlinStringLiteral), serializer = serializer())"
            )
        }
    ...
    func nextPageMethodImplementation() -> any Node {
        let responseType = operation.response.dataType.map { dataType in
            ...
            return "ApiResponseType<\(typeDeclaration)>(typeName = \(typeDeclaration.kotlinStringLiteral), mimeType = \(operation.response.mimeType.kotlinStringLiteral), serializer = serializer())"
        } ?? "ApiResponseType<Unit>(typeName = \"Unit\", mimeType = \(operation.response.mimeType.kotlinStringLiteral), serializer = serializer())"
        let requestArgument = operation.security == .unsecured ? "request" : "adaptedRequest"

        return Block {
            "override \(nextPageMethodDeclaration(includeDefaults: false)) {"
            Indentation {
                requestConstruction()
    ...
    private func mapAssignment(for parameter: ApiParameter) -> String {
        ...
        if parameter.dataType.isKotlinAndroidOperationCollection {
...
struct KotlinAndroidApiServiceEmitter {
    let definition: ApiService
    let moduleName: String
    let options: KotlinAndroidGeneratorOptions

    init(definition: ApiService, moduleName: String, options: KotlinAndroidGeneratorOptions = .init()) {
    ...
    func serviceDeclaration() -> any Node {
        let pagedResultsOperations = definition.operations.filter {
            KotlinAndroidOperationEmitter(operation: $0, options: options).hasPagedResultsResponse
        }
        ...
        return Block {
            ...
            Indentation {
                (
                    definition.operations.map {
                        KotlinAndroidOperationEmitter(operation: $0, options: options).serviceMethodDeclaration() as any Node
                    }
                        + pagedResultsOperations.map {
                            KotlinAndroidOperationEmitter(operation: $0, options: options).nextPageMethodDeclaration() as any Node
                        }
                )
                .map { $0.toString() }
                .joined(separator: "\n\n")
            ...
            Indentation {
                (
                    definition.operations.map {
                        KotlinAndroidOperationEmitter(operation: $0, options: options).serviceMethodImplementation() as any Node
                    }
                        + pagedResultsOperations.map {
                            KotlinAndroidOperationEmitter(operation: $0, options: options).nextPageMethodImplementation()
                        }
                )
                .map { $0.toString() }
                .joined(separator: "\n\n")
...
struct KotlinAndroidApiAggregateServiceEmitter {
...
enum KotlinAndroidFileEmitter {
    ...
    static func renderNode(packageName: String?, imports: [String], body: any Node) -> any Node {
        var nodes: [any Node] = []
        nodes.append(KotlinAndroidGeneratedTextFile.managedHeader)
        if let packageName {
...
private extension ApiRequestBody {
    ...
    var requiresProgress: Bool {
        isMultipart || isUpload || isBinary
    }

    func bodyTypeExpression(options: KotlinAndroidGeneratorOptions) -> String {
        switch self {
            case let .json(type?):
                "apiBodySerializer<\(type.kotlinTypeDeclaration(options: options))>()"
            case .none,
                 .binary,
                 .file,
                 .multiPart,
...
private extension ApiOperation {
    func kotlinOperationImports(options: KotlinAndroidGeneratorOptions) -> Set<String> {
