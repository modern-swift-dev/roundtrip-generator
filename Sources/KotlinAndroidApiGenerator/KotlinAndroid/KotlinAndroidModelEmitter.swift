...
struct KotlinAndroidModelEmitter {
    let dataType: ApiTypeSchema
    let options: KotlinAndroidGeneratorOptions
    let excludedNestedTypeIDs: Set<UUID>

    init(dataType: ApiTypeSchema, options: KotlinAndroidGeneratorOptions = .init(), excludedNestedTypeIDs: Set<UUID> = []) {
    ...
    var declaration: String? {
        switch dataType {
            case let .object(typeName, properties, protocols, _, isValueType, _):
                ...
                let usesPatchableValue = fields.contains { $0.dataType.isKotlinAndroidPatchableValue }
                let modelProperties = fields.map { field in
                    var property = field.kotlinProperty(options: options)
                    property.mutable = field.dataType.isKotlinAndroidPatchableValue
            ...
            case let .stringEnum(typeName, values, _, _, supportGarbage):
                ...
                return KotlinAndroidEnumClass(
                    name: typeName.kotlinTypeName,
                    cases: cases,
                    rawValuePropertyName: "rawValue",
                    rawValueType: KotlinAndroidTypeName("String"),
            ...
            case let .intEnum(typeName, values, _, _):
                let cases = values.map { value in
                    ...
                    return KotlinAndroidEnumCase(name: name, arguments: [String(value.rawValue)])
                }
                ...
                return KotlinAndroidEnumClass(
                    ...
                    cases: cases,
                    rawValuePropertyName: "rawValue",
                    rawValueType: KotlinAndroidTypeName("Int"),
            ...
            extraProperties,
                ...
                let cases = objectTypes.map { objectType -> any Node in
                    ...
                    let payloadType = KotlinAndroidTypeEmitter(dataType: objectType.objectType, options: options).typeName
                    let payloadParam = KotlinAndroidProperty(
                        name: "payload",
    ...
    private static func patchableDeclarations(properties: [ApiModelProperty]) -> [String] {
        let patchableProperties = properties.filter(\.dataType.isKotlinAndroidPatchableValue)
        ...
        switch dataType {
            ...
            case let .dynamicObject(_, _, objectDataPropertyName, alternateObjectDataPropertyName, objectTypes, supportGarbage, _, _, extraProperties):
                ...
                let payloadImports = objectTypes
                    .map { KotlinAndroidTypeEmitter(dataType: $0.objectType, options: options).typeName.imports }
                    .reduce(Set<String>()) { $0.union($1) }
    ...
    private static func nestedEmitters(
        ...
        switch dataType {
            case .object,
                 ...
                 .intEnum,
                 .dynamicObject:
                return [KotlinAndroidModelEmitter(dataType: dataType, options: options, excludedNestedTypeIDs: excludedTypeIDs)]
            case let .array(type),
