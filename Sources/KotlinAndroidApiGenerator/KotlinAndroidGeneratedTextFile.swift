...
public enum KotlinAndroidGeneratedTextFileError: Error, Equatable, LocalizedError {
    ...
    public var errorDescription: String? {
        switch self {
            case let .invalidRelativePath(path):
                "Invalid generated KotlinAndroid file path: \(path)"
            case let .refusingToOverwriteUserFile(path):
                "Refusing to overwrite user-owned KotlinAndroid file: \(path)"
        }
    }
}

public struct KotlinAndroidGeneratedTextFile: Sendable, Equatable {
    ...
    public func write(to directoryURL: URL, overwritePolicy: KotlinAndroidGeneratedFileOverwritePolicy) throws {
        let components = try validatedPathComponents()
        let fileURL = components.reduce(directoryURL) { partialURL, component in
            partialURL.appendingPathComponent(component)
        }
        guard Self.isContained(fileURL, in: directoryURL),
              Self.isContained(fileURL.deletingLastPathComponent(), in: directoryURL) else {
            throw KotlinAndroidGeneratedTextFileError.invalidRelativePath(relativePath)
        }

        if FileManager.default.fileExists(atPath: fileURL.path) {
            switch overwritePolicy {
                case .neverOverwriteExisting:
                    throw KotlinAndroidGeneratedTextFileError.refusingToOverwriteUserFile(relativePath)
