...
public extension ApiTypeSchema {
    ...
    func prepending(properties newProperties: [ApiModelProperty], newImports: [ApiImport] = []) throws -> ApiTypeSchema {
        switch self {
            ...
            case let .keyedByString(type, isOptional):
                try .keyedByString(type.prepending(properties: newProperties), isOptional: isOptional)
            case let .array(type):
                try .array(type.prepending(properties: newProperties))
            default:
    ...
    func appending(properties newProperties: [ApiModelProperty], newImports: [ApiImport] = []) throws -> ApiTypeSchema {
        switch self {
            ...
            case let .keyedByString(type, isOptional):
                try .keyedByString(type.appending(properties: newProperties), isOptional: isOptional)
            case let .array(type):
                try .array(type.appending(properties: newProperties))
            default:
                throw ApiValidationError.failed("Impossible to add properties to a non-object data type")
        }
    }
    ...
    func modifyingProperties(modifier: (ApiModelProperty) -> ApiModelProperty) throws -> ApiTypeSchema {
        switch self {
            ...
            case let .keyedByString(type, isOptional):
                try .keyedByString(type.modifyingProperties(modifier: modifier), isOptional: isOptional)
            case let .array(type):
                try .array(type.modifyingProperties(modifier: modifier))
            default:
                throw ApiValidationError.failed("Impossible to add properties to a non-object data type")
        }
    }
    ...
    func implementing(protocols newProtocols: [String]) throws -> ApiTypeSchema {
        switch self {
            ...
            case let .keyedByString(type, isOptional):
                try .keyedByString(type.implementing(protocols: newProtocols), isOptional: isOptional)
            case let .array(type):
                try .array(type.implementing(protocols: newProtocols))
            default:
                throw ApiValidationError.failed("Impossible to add properties to a non-object data type")
        }
    }
