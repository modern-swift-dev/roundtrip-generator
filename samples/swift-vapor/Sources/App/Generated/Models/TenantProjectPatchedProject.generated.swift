...
public struct TenantProjectPatchedProject: Codable, Sendable {
    ...
    public init(from decoder: any Decoder) throws {
        ...
        status = try container.decodePatchable(PatchableValue<TenantStatus>.self, forKey: .status)
        owners = try container.decodePatchable(PatchableValue<[NamedObject]>.self, forKey: .owners)
        labels = try container.decodePatchable(PatchableValue<[String: String?]>.self, forKey: .labels)
    }

    public func encode(to encoder: any Encoder) throws {
        var container = encoder.container(keyedBy: CodingKeys.self)
        try container.encodePatchable(name, forKey: .name)
        try container.encodePatchable(status, forKey: .status)
        try container.encodePatchable(owners, forKey: .owners)
        try container.encodePatchable(labels, forKey: .labels)
    }
}
