        ...
        public struct Request: URLRequestConvertible, Equatable, Sendable {
            ...
            public var requestPath: String {
                ...
                path = path.replacingOccurrences(
                    of: "{matrix_id}",
                    with: String(self.matrixId).addingPercentEncoding(withAllowedCharacters: pathSegmentAllowedCharacters) ?? String(self.matrixId)
                )
                return path
            }
            public var httpHeaders: [String: String] {
                var values: [String: String] = [:]
                if let __api0TraceIdValue = self.traceId {
                    values["X-Trace-Id"] = __api0TraceIdValue
                }
                if let __api1ApiKeyValue = self.apiKey {
            ...
            public var httpCookies: [String: any FormEncodable] {
                ...
                if let __api0SampleSessionValue = self.sampleSession {
            ...
            public var queryParameters: [String: any FormEncodable] {
                var values: [String: any FormEncodable] = [:]
                let __api0VisibleValue = self.visible
                values["visible"] = String(describing: __api0VisibleValue)
                let __api1VisibilityValue = self.visibility
                values["visibility"] = __api1VisibilityValue.rawValue
                if let __api2ScoresValue = self.scores, !__api2ScoresValue.isEmpty {
            ...
            public init(matrixId: String, visible: Bool = true, visibility: ShowcaseApi.SampleVisibility = .`public`, scores: [SampleScore]? = [.high], traceId: String? = nil, sampleSession: String? = nil, apiKey: String? = nil) {
