...
public extension LocationStructureApi {
    struct PatchOperation: Sendable {
        public struct Request: URLRequestConvertible, Sendable {
            ...
            public var body: PatchedStructure
            public var requestPath: String {
                ...
                path = path.replacingOccurrences(
                    of: "{structure_id}",
                    with: String(self.structureId).addingPercentEncoding(withAllowedCharacters: pathSegmentAllowedCharacters) ?? String(self.structureId)
                )
                return path
            }

            public init(structureId: Int64, body: PatchedStructure) {
